...
void update(int tree[][3], int lazy[], int n, int si, int ss, int se, int rs, int re, int add, bool nolazy=false){
  ...
  if(ss==se){
    ...
    //print(tree, n);
    //printlazy(lazy, n);
    //printf("tree[%d][0] = %d", si, (tree[si][0] + add)%3);
    //print(tree, n);
    //printlazy(lazy, n);
    //cout << endl;
    return;
  ...
  if(si*2>=n){ 
    ...
    //cout << "s-10 " << si << endl;
    //printf("tree -> 0 - %d | 1 - %d | 2 - %d \n", tree[si*2][0] + tree[si*2+1][0], tree[si*2][1] + tree[si*2+1][1], tree[si*2][2] + tree[si*2+1][2]);
  }
}
